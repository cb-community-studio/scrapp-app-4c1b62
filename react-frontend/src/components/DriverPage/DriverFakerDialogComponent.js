...
import driverFakerFactory from "./driverFakerFactory";
...
const DriverFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = driverFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("driver").create(elem));
...
export default connect(null, mapDispatch)(DriverFakerDialogComponent);
